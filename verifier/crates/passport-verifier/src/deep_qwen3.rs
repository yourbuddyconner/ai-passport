//! Vendored from candle-transformers 0.9.2 with an f16 embedding table
//! so Qwen3's 151k-token vocab fits a 1 GB enclave.
...
impl ModelWeights {
    pub fn from_gguf<R: Read + Seek>(
        ...
        // Patched: keep the huge vocab embedding in f16 — halves resident memory
        // for 1 GB enclaves. Lookups are cast back to f32 in forward().
    ...
    pub fn forward(&mut self, input: &Tensor, offset: usize) -> Result<Tensor> {
        let _enter = self.span.enter();
        let (b, l) = input.dims2()?;
        let mut h = self.embed_tokens.forward(input)?.to_dtype(DType::F32)?;
